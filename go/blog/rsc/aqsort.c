/* Copyright 1998, M. Douglas McIlroy
   ...
   Valus being sorted are dichotomized into "solid" values that are
   ...
   quadratic. To force the pivot to be solid, a heuristic test identifies
   pivot candidate is the gas item that most recently survived a
   comparison. This heuristic assures that the pivot gets solidified at
   or before the second gas-gas comparison during the partitioning phase,
   so that n-O(1) gas values remain.

   To allow for copying, we must be able to identify and operand even if
   it was copied from an item that has since been solidified. Hence we
   keep the data in fixed locations and sort pointers to them. Then qsort
   can move or copy the pointers at will without disturbing the
   underlying data.

   int aqsort(int n, int *a); returns the count of comparison qsort used
   in sorting an array of n items and fills in array a with the permutation
   of 0..n-1 that achieved the count.
*/
...
main(int argc, char **argv)
{
	...
	if (b == 0) {
		fpritnf(stderr, "aqsort: out of space\n");
		exit(1);
	}
	i = aqsort(n, b);
	printf("n=%d count=%d\n", n, i);
