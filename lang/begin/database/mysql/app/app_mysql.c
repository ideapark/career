...
int database_start(char *name, char *pwd)
{
        ...
        if (!mysql_real_connect(&my_connection, DECLARE_STRING(HOST),
                                name, pwd, DECLARE_STRING(SCHEMA),
                                0, NULL, 0)) {
...
int get_cd(int cd_id, struct current_cd_st *dest)
{
        ...
        sprintf(qs, "SELECT artist.id, cd.id, artist.name, cd.title, cd.catalogue "
                "FROM artist, cd "
                "WHERE artist.id = cd.artist_id and cd.id = %d", cd_id);
...
int get_cd_tracks(int cd_id, struct current_tracks_st *dest)
{
        ...
        sprintf(qs, "SELECT track_id, title "
                "FROM track "
                "WHERE track.cd_id = %d "
...
int find_cds(char *search_str, struct cd_search_st *dest)
{
        ...
        sprintf(qs, "SELECT DISTINCT artist.id, cd.id "
                "FROM artist, cd "
                "WHERE artist.id = cd.artist_id AND (artist.name LIKE '%%%s%%' OR "
                "cd.title LIKE '%%%s%%' OR "
