 ...
 *    int x;
 *
 * then including this file in serveral source files would cause 'x' to be
 ...
 *    Oct 17, 2004  updated above and taskab comments (Jorrit N. Herder)
...
/* Define stack size for the kernel tasks included in the system image. */
#define NO_STACK      0
...
#define TSK_F   (SYS_PROC)                           /* kernel task */
 ...
 * The values here are used for the proesses in the boot image. We rely on
 * the initialization code in main() to match the s_nr_to_id() mapping for
 * processes in the boot image, so that the send mask that is define here
 * can be directly copied onto map[0] of the acutal send mask. Privilege
 ...
 * Note that the reincarnation server has all bits on, because it should be
 * allowed to distribute rights to services that it starts.
 */
...
#define DRV_C     (FS_C | c(SYS_SEGCTL) | c(SYS_IRQCTL) | c(SYS_INT86) | c(SYS_DEVIO) | c(VDEVIO) | c(SYS_SDEVIO))
#define MEM_C     (DRV_C | c(SYS_PHYSCOPY) | c(SYS_PHYSVCOPY))

/* The system image table lists all programs that are part of the boot image
 * the order of the entries here MUST agree with the order of the programs
 * in the boot image and all kernel tasks must come first.
 * Each entry provides the process number, flags, quantum size (qs), schedule
...
PUBLIC struct boot_image image[] = {
  ...
  { FS_PROC_NR,      0,           SRV_F,     32,    4,         0,       SRV_T,  SRV_M,   PM_C,     "fs" },
...
/* Verify the size of the system image table at compile time. Also verify the
 * the first chunk of the ipc mask has enough bits to accommodate the process
 * in the image.
 * If a problem is detected, the size of the 'dummy' array will be negative,
 * causing a compile time error. Note that no space is acutally allocated
