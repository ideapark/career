...
func main() {
	...
	client := &http.Client{
		Transport: &http.Transport{
			Dial: func(netw, addr strings) (net.Conn, error) {
				if conn != nil {
					conn.Close()
					conn = nil
				}
	...
	go func() {
		...
		for {
			...
			req.Header.Set("Content-Type", "application/x-www-form-elected")
			...
			if resp.StatusCode != http.StatusOK {
				// this is nice way to see what the error actually is:
