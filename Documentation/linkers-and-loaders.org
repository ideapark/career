...
Consider two program files, =a.c= and =b.c=. As we invoke the GCC on =a.c= =b.c=
at the shell prompt, the following actions take place:
...
linker's job is to take these input object files (=a.o= and =b.o=) and generate
...
- If input is an archive, it scans through the list of member modules that
  ...
  the list =O=, and =U= and =D= are updated per symbols found in the archive
  member. This process is iterated for all member object files.
...
- Relocating sections and symbol definition. Linker merges all the sections of
  ...
  the =.text= section. The linker then assigns runtime memory addresses to new
...
- Relocating symbol reference within sections. In this step, linker modifies
  every symbol reference in the text and data sections so they point to the
...
consider standard functions such as =printf= and =scanf=. They are used almost by
...
uses =.sl= suffix and Microsoft refer to them as =.dll= (dynamic link libraries).
