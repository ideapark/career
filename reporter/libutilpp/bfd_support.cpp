...
void bfd_info::get_symbols()
{
  ...
  /*
   * If dynamic library and no symbols in .symtab, get symbols
   * from .dynsym section.
   */
  if (0 == nr_syms) {
    cverb << vbfd << "no symbols in section .symtab, get symbol from .dynsym instead." << endl;

    nr_syms = bfd_get_dynamic_symtab_upper_bound(abfd);
    cverb << vbfd << "bfd_get_dynamic_symtab_upper_bound: " << dec << nr_syms << hex << endl;
    ...
    nr_syms = bfd_canonicalize_dynamic_symtab(abfd, syms.get());
  }
  if (image_bfd_info)
