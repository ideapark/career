...
{
	struct record_entry callee_record = record, caller_record = record;
	sym_info callee_sym_info, caller_sym_info;
			       ...
			       bool deduced /* deduced from call stack? */)
{
	 ...
	 * func_info
	 */
	key = to_string(sym_info.bfd_key) + "|" + to_string(sym_info.vma_start);
	 ...
	 */
	if (record.time_diff == 0 || record.time_diff == (summaryInfo.end_time - summaryInfo.start_time))
		return;
	...
	while (e_iter != e_end) {
		...
		while (pi_iter != pi_end) {
			...
			if (process_info->event_private_key == event_info->event_private_key) {
				...
				process_info->sample_ratio = (process_info->sample_count * 100.0) / theory_ce;
					...
					if (task_info->event_private_key == event_info->event_private_key &&
						...
						while (ts_iter != ts_end) {
							...
							if (task_slice->event_private_key == event_info->event_private_key &&
								...
								task_info->peak_ratio = std::max<float>(task_info->peak_ratio,
