...
op_bfd_symbol::op_bfd_symbol(bfd_vma section_file, bfd_vma section_file_vma,
                             size_t size, std::string const & name)
  : bfd_symbol(0), symb_value(0),
    section_filepos(section_file), section_vma(section_file_vma),
    symb_size(size), symb_name(name),
    symb_artificial(true)
{
}
    ...
    anon_obj(false)
{
  ...
  // O(N2) behavior when we will filter vector element below
...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//              Added by APS [Advanced Performance Solution]                  //
//                                                                            //
////////////////////////////////////////////////////////////////////////////////
    ...
    is_vmlinux(false)
{
  ...
  const string suf = ".jo";
  const string libso = ".so";
  const string vmlinux = "vmlinux";

  char *basename_string = strdup(image_path.c_str());
  this->filename = basename(basename_string);
  free(basename_string);

  char *dirname_string = strdup(image_path.c_str());
  this->archive_path = dirname(dirname_string);
  free(dirname_string);
  ...
  pos = filename.rfind(libso);
  if (pos != string::npos && pos == filename.size() - libso.size())
    is_libso = true;

  pos = filename.rfind(vmlinux);
  if (pos != string::npos && pos == filename.size() - vmlinux.size())
    is_vmlinux = true;
...
symbol_index_t op_bfd::get_symindex(unsigned long long file_offset) const
{
  ...
  return APS_INVALID_SYMBOL_INDEX;
}


unsigned long op_bfd::start_vma(symbol_index_t index) const
{
...
string op_bfd::get_symname(symbol_index_t index) const
{
  if (index == APS_INVALID_SYMBOL_INDEX)
    return "";

  return this->syms[index].name();
}
