...
;;
;; Author: Bastien / @bgz2 / http://bzg.fr
;;
...
;; Import warning:
;;
;; Goring through this tutorial won't damage your computer unless
;; you get to angry that you throw it on the floor. In that case,
...
;; specific mode. Here we use the `list-interaction-mode`, which
...
;; Let's make our `greeting` function prompt for you name:
...
;; side-effects (like `car`) while other have (like `push`).
...
;; argument, a name. `mapcar` calls `hallo`, successively using each
...
;; (search-forward "Hello" nil 't) does the work:
...
;; `add-text-properties` adds... text properties, like the face.
