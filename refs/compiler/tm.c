...
void writeInstruction(int loc)
{
	...
	if ((loc >= 0) && (loc < IADDR_SIZE)) {
		...
		case opclRR:
			printf("%d,%d", iMem[loc].iarg2, iMem[loc].iarg3);
			break;
		case opclRM:
		case opclRA:
	...
	while (!feof(pgm)) {
		...
		if ((nonBlank()) && (in_Line[inCol] != '*')) {
			...
			if (strncmp(opCodeTab[op], word, 4) != 0)
				return error("Illegal opcde", lineNo, loc);
			switch (opClass(op)) {
			case opclRR:
...
int doCommand(void)
{
	...
	case 'h':
		printf("Commands are:\n");
		printf("    s(step <n>      Execute n (default 1) TM instructions\n");
