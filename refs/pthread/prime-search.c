 ...
 * Global data
 */
pthread_mutex_t prime_list    = PTHREAD_MUTEX_INITIALIZER;  /* Mutex for use in accessing the prime */
pthread_mutex_t current_mutex = PTHREAD_MUTEX_INITIALIZER;  /* Mutex associated with current number */
pthread_mutex_t cond_mutex    = PTHREAD_MUTEX_INITIALIZER;  /* Mutex used for thread start */
pthread_cond_t cond_var       = PTHREAD_COND_INITIALIZER;  /* Condition variable for thread start */

int current_num = -1; /* Next number to be checked, start odd */
int thread_hold = 1;  /* Number associated with condition state */
int count       = 1;  /* Count of prime numbers - index to primes */
...
void *prime_search(void *arg)
{
	 ...
	 * the predicate of which (thread_hold) will be set by the parent.
	 */
...
int main(void)
{
	 ...
	 * successfully.
	 *
	 * Mark thread storage free to be reclaimed upon termination
	 * by detaching it.
	 */
	for (worker_num = 0; worker_num < workers; worker_num++) {
	 ...
	 * concurrently; threre is no guarantee that the prime numbers
