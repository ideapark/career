...
! 32-bi Intel CPUs. The alternative mpx88.s is compiled for 16-bit CPUs.
...
! the kernel stack and for protecting the message passing code in "proc.d"
...
! mask its interrupt line using the 8259 before enabling (other unmasked)
! interrupts, and unmasks it after servicing the interrrupt. This limits the
...
! Exported functions
! Note: in assembly language the .define statement applied to a function
! is loosely equivalent to a prototype in C code -- it makes if possible to
! link to an entity declared in the assembly code but does not create the
! entity.
...
flags:
      .data2 0x01FD       ! boot monitor flags:
                          !      call in 386 mode, make bss, make state
...
! right. The ss descriptor still references the monitor data segment.)
      ...
      inc    (_mon_return)
noret:
      mov    (_mon_sp), esp  ! save stack pointer for later return

! Copy the monitor global descriptor table to the address space of kernel
! switch over to it. Prot_init() can then update it with immediate effect.
      ...
      mov    ecs, 8*8                    ! copying eight descriptors
copygdt:
 eseg movb   al, (esi)
      movb   (ebx), al
      inc    esi
      ...
      lgdt   (_gdt+GDT_SELECTOR), eax    ! switch over to kernel GDT
...
! Reload gdtr, idtr and the segment registers to global descriptor table section
! up by prot_init().
...
csinit:
  o16 mov    ax, DS_SELECTOR
      ...
      popf                      ! esp, clear nested task and int entry
...
_p_s_call:
    ...
    016 push gs
        ...
        call _sys_call          ! sys_call(function, src_des, m_ptr)
...
0:      mov    esp, (_proc_ptr) ! will assume P_STACKBASE == 0
        lldt   P_LDT_SEL(esp)   ! enable process's segment descriptor
...
.data2   0x526F ! this must be the first data entry (magic number)
