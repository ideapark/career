...
void TextQuery::retrieve_text() {
    ...
    if (!infile) {
        cerr << "oops! unable to open file " << file_name
             << " -- bailing out!\n";
        exit(-1);
    } else {
        cout << "\n";
...
void TextQuery::display_text_locations() {
    vector<string> *text_words = text_locations->first;
    loc *text_locs = text_locations->second;
    register int elem_cnt = text_words->size();
    if (elem_cnt != text_locs->size()) {
        cerr << "oops! internal error: words and position vectors "
             << "are of unequal size \n"
             << "words: " << elem_cnt << " "
             << "locs: " << text_locs->size()
             << " -- bailing out!\n";
