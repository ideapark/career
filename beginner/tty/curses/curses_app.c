...
const char *title_file = "title.db";
const char *tracks_file = "tracks.db";
    ...
    do {
        ...
        switch (choice) {
            ...
            case 'u':
                ...
                break;
            default:
                fprintf(stderr, "FATAL - this can not happy!!!\n");
                exit(-1);
        }
    } while (choice != 'q');
...
void insert_title(char *cdtitle)
{
    ...
    if (!fp)
        mvprintw(ERROR_LINE, 0, "cannot open CD titles databases");
    else {
        fprintf(fp, "%s\n", cdtitle);
        fclose(fp);
    }
...
void count_cds()
{
    ...
    mvprintw(ERROR_LINE, 0,
            "Database contain %d titles, with a total of %d tracks.",
            titles, tracks);
    get_return();
}
...
void list_tracks()
{
    ...
    /* make a new pad, ensure that even if there is only a single
       track the PAD is large enough so the later prefresh() is
       always valid. */
