 ...
 * and which are not. The code for unneeded kernel calls is not included
 * in the system binary, making it smaller. If you are not sure, it is
 * best to keep all kernel calls enabled.
 */
...
#define USE_DEVIO         1  /* read or write a signal I/O port */
...
/* This section allows to enable kernel debugging and timing functionally.
 * For normal operation all options should be disabled.
 */
#define DEBUG_SCHED_CHECK  0  /* sanity check of scheduling queues */
#define DEBUG_LOCK_CHECK   0  /* measure time spent in locks */
