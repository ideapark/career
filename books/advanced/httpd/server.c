 ...
 * request document was not found.
 */
...
/*
 * Handle a client connectoin on the file descriptor CONNECTION_FD.
 */
static void handle_connection(int connection_fd)
{
	...
	if (bytes_read > 0) {
		...
		/* The first line the client sends is the HTTP request, which is
		   compose of a method, the requested page, and the protocol version. */
		...
		if (strcmp(protocol, "HTTP/1.0") && strcmp(protocol, "HTTP/1.1")) {
			...
			/* This sever only implements the GET method. The client
...
void server_run(struct in_addr local_address, uint16_t port)
{
	...
	while (1) {
		...
		if (connection == -1) {
			...
			if (errno == EINTR)
				/* The call was interrupted by a singal. Try again. */
				continue;
			else
				system_error("accept");
		}
