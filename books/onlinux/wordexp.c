	...
	for (i = 1; i < argc; i++) {
		...
		if (ret != 0) {
			fprintf(stderr, "%s: problem with %s (%s), stopping early\n",
					myname, argv[i],
					(ret == WRDE_BADCHAR ? "metacharacter invalid location" :
					 ret == WRDE_BADVAL ? "variable undefined" :
					 ret == WRDE_CMDSUB ? "attempte to substitution" :
					 ret == WRDE_NOSPACE ? "memory allocating error" :
					 ret == WRDE_SYNTAX ? "shell syntax error" :
					 "unknown problem")
			       );
			break;
		}
